(() => {
  ...
  function ensureStyles(doc) {
    ...
    style.textContent = `
      ...
      .rpr-shell{min-height:calc(100vh - 54px - env(safe-area-inset-top) - env(safe-area-inset-bottom));display:flex;flex-direction:column}
      .rpr-top{display:flex;align-items:center;justify-content:space-between;min-height:38px}.rpr-logo{width:104px;height:35px;object-fit:contain;object-position:left center}.rpr-step{font-size:11px;font-weight:600;color:#8A9491}
  ...
  function renderMagic(win) {
    const db = readDb(win), day = dayState(db, todayKey()), action = activeAction(day);
    if (!action) return `<div class="rpr-stage"><div class="rpr-eyebrow">Подбираем шаг</div><h1 class="rpr-title">Секунду…</h1><p class="rpr-lead">Rinlo собирает первый вариант из выбранной цели и сегодняшнего состояния.</p></div>`;
  ...
  function renderOnboarding(doc, win) {
    ...
    const stepText = draft.step === 0 ? '' : draft.step === 1 ? '1 из 2' : draft.step === 2 ? '2 из 2' : '';
    const body = draft.step === 0 ? renderWelcome(doc) : draft.step === 1 ? renderGoals() : draft.step === 2 ? renderContext() : renderMagic(win);
    el.innerHTML = `<div class="rpr-shell"><div class="rpr-top"><img class="rpr-logo" src="./rinlo-logo.svg?rev=31" alt="Rinlo"><span class="rpr-step">${stepText}</span></div>${body}</div>`;
    doc.querySelectorAll('.screen').forEach((screen) => { if (screen !== el) screen.classList.remove('on'); });
    const nav = doc.getElementById('nav'); if (nav) nav.style.display = 'none';
    const fab = doc.getElementById('fab'); if (fab) fab.style.display = 'none';
  }
  ...
  function applyToday(doc, win) {
    ...
    if (greeting && !today.querySelector('.rpr-day-promise')) greeting.insertAdjacentHTML('afterend', '<div class="rpr-day-promise">Rinlo выбирает один следующий шаг. Остальные данные нужны только как контекст.</div>');

    const checkin = doc.getElementById('rcCheckin');
    const action = doc.getElementById('rcAction');
    if (checkin && action) {
      if (currentCheckin(win)) {
        action.parentNode?.insertBefore(action, checkin);
        checkin.classList.add('rpr-checkin-compact');
      } else {
        checkin.parentNode?.insertBefore(checkin, action);
        checkin.classList.remove('rpr-checkin-compact');
      }
    }
    const actionCard = action?.querySelector('.rc-action');
    if (actionCard) {
      const kicker = actionCard.querySelector('.rc-action-kicker');
      if (kicker) kicker.textContent = 'Сейчас лучше всего';
    ...
    if (quickSection?.classList.contains('rc-section')) {
      const title = quickSection.querySelector('h2'); if (title) title.textContent = 'Добавить контекст';
      const hint = quickSection.querySelector('span'); if (hint) hint.textContent = 'необязательно';
    }

    const metrics = doc.getElementById('rcMetrics');
    if (metrics) metrics.style.display = profile.detailsComplete === false ? 'none' : '';
    let precision = today.querySelector('.rpr-precision');
    ...
    } else if (profile.detailsComplete !== false) precision?.remove();
  }
  ...
  function applyProfile(doc, win) {
    ...
    const grid = root.querySelector('.rpf-context-grid'); if (grid) grid.style.display = 'none';
    const contextCopy = root.querySelector('.rpf-context-copy');
    if (contextCopy) contextCopy.innerHTML = '<b>Личный контекст</b><p>Подробные параметры пока не заполнены — и это нормально.</p>';
    const pGoal = root.querySelector('#pGoal'); if (pGoal) pGoal.textContent = goalNames[profile.primaryGoal] || 'Основная цель выбрана';
    const pTargets = root.querySelector('#pTargets'); if (pTargets) pTargets.textContent = 'Добавьте параметры, если захотите видеть числовые ориентиры';
  }
  ...
  async function waitForAction(win, timeout = 2500) {
  ...
  function expose(win, doc) {
    ...
    win.rinloProductCreateFirst = async () => {
      ...
      draft.step = 3; renderOnboarding(doc, win);
    };
    win.rinloProductAnother = async () => { await win.rinloCoreReplaceAction?.(); await waitForAction(win); renderOnboarding(doc, win); };
  ...
  function mount(attempt = 0) {
    ...
    if (mountedDocument !== doc) {
      observer?.disconnect();
      mountedDocument = doc;
      ensureStyles(doc); expose(win, doc); installBudgetBridge(win); wrapToday(win, doc);
      observer = new MutationObserver(() => {
        const db = readDb(win);
        if (!db.profile && draft.step < 4 && !doc.querySelector('.rpr-onboarding')) renderOnboarding(doc, win);
        if (db.profile) { applyToday(doc, win); applyProfile(doc, win); }
      });
      observer.observe(doc.documentElement, { childList:true, subtree:true });
    }
    ensureStyles(doc); expose(win, doc); installBudgetBridge(win); wrapToday(win, doc);
    if (!readDb(win).profile) renderOnboarding(doc, win);
    else { applyToday(doc, win); applyProfile(doc, win); }
  }
