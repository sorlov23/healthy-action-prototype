(() => {
  ...
  function renderToday(doc, win) {
    ...
    const timeline = `<div class="r3-section"><h2>За сегодня</h2><span>${events.length?`${events.length} последних`:'пока пусто'}</span></div><div class="r3-timeline" data-testid="today-timeline">${events.length?events.map((event)=>`<div class="r3-event"><span class="r3-event-time">${new Date(event.time).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}</span><div class="r3-event-copy"><b>${event.type==='food'?esc(event.text):'Вес'}</b><span>${event.type==='food'?`≈ ${Math.round(event.cal||0)} ккал · ${Math.round(event.protein||0)} г белка`:`${event.weight} кг`}</span></div><button class="r3-event-delete" aria-label="Удалить" onclick="delEvent(${Number(event.id)})">×</button></div>`).join(''):`<div class="r3-empty">Записи появятся здесь по ходу дня. Заполнять всё подряд не нужно.</div>`}</div>`;
    const showEvening = isToday && (day.closed || hour >= 18 || action?.status === 'completed');
    const evening = showEvening ? `<div class="r3-soft"><div class="r3-soft-icon">${icon.moon}</div><div class="r3-soft-copy"><b>${day.closed?'День завершён':'Закончить день'}</b><span>${day.closed?'Итог сохранён. Завтра начнём с нового шага.':'Два коротких ответа — и на сегодня всё.'}</span></div>${day.closed?'':`<button onclick="finishDay()">Подвести итог</button>`}</div>` : '';
  ...
  function renderProgress(doc, win) {
    ...
    const db = readDb(win), profile=db.profile||{}, rows=weekData(db), active=rows.filter((r)=>r.active).length, meals=rows.reduce((n,r)=>n+r.sum.meals,0), avgSteps=Math.round(rows.reduce((n,r)=>n+Number(r.day?.steps||0),0)/7);
    const waterDays=rows.filter((r)=>Number(r.day?.water||0)>0).length, movementDays=rows.filter((r)=>Number(r.day?.steps||0)>0).length, foodDays=rows.filter((r)=>r.sum.meals>0).length;
